 ...
 * occurs to an extra in the database, and reconstruct it by a call to the 
 * constructor. The methods of the class handles removal of existing extras 
 * from the database, as well as adding new extras to the database.
 * 
...
public class ExtraList {
	...
	public ExtraList(DatabaseConnection dbCon) throws SQLException {
	...
	/**
	 * Adds an extra to the database (e.g. extra bacon, without onions,
	 * large pizza)
	 * 
	 * @param dbCon
	 ...
	 * @param price
	 *            the price modifier of the extra as a string, where the
	...
	//TODO: Add validation to ensure the price parameter is in the correct format	
	
	public boolean addExtra(DatabaseConnection dbCon, String name, String price) {
		if (!(dbCon != null && dbCon.isConnected(DatabaseConnection.DEFAULT_TIMEOUT))) {
			System.err.println("No valid database connection specified; extra not added to the database.");
			return false;
		}
		if (name.length() > DatabaseConnection.VARCHAR_MAX_LENGTH_LONG) {
			throw new IllegalArgumentException("The name of the extra cannot be more than "
							+ DatabaseConnection.VARCHAR_MAX_LENGTH_LONG + " characters long.");
		}
		return dbCon.insertIntoDB("INSERT IGNORE INTO Extras (Name, Price) VALUES ('"
				+ name + "', '" + price + "');");
	}
	...
	public boolean removeExtra(DatabaseConnection dbCon, Extra extra) {
		if (!(dbCon != null && dbCon.isConnected(DatabaseConnection.DEFAULT_TIMEOUT))) {
			System.err.println("No valid database connection specified; no extra removed from the database.");
			return false;
		}
		return dbCon.insertIntoDB("DELETE FROM Extras WHERE ExtrasID=" + extra.id + ");");
	}
}