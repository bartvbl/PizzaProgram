...
public class CookGUI_CookViewEventHandler extends ComponentEventHandler implements ActionListener {
	...
	/**
	 * Registers all appropiate event handlers at the different components in
	 * the cook view
	 * 
	 * @param cookGUI
	 *            A reference to the main CookGUI module
	 */
	...
	/**
	 * Registers event listeners at the various components in the cook GUI that
	 * this class handles
	 */
	private void addEventListeners() {
		...
		CookView.orderSearchTextPane.addFocusListener(new FocusListener() {
	...
	/**
	 * Handles events from the Cook view, and calls the appropiate event
	 * handling function
	 * 
	 * @param event
	...
	/**
	 * Shows all customers in the cook gui if the search box is empty, otherwise
	 * it searches for the contents of the search box in the database
	 */
	...
	/**
	 * Automatically selects all text in the order search text box if the user
	 * gives it focus.
	 */
	...
	/**
	 * Shows all orders that have not yet finished cooking (which it fetches
	 * from the database)
	 */
	...
	private void markOrderCompleted() {
		this.dispatchEvent(new Event<Order>(EventType.DATABASE_MARK_ORDER_FINISHED_COOKING,
				this.cookGUI.currentSelectedOrder));
		this.resetUI();
	...
	private void markOrderAsInProgress() {
		this.dispatchEvent(new Event<Order>(EventType.DATABASE_MARK_ORDER_IN_PROGRESS,
				this.cookGUI.currentSelectedOrder));
		this.updateCurrentOrder();
	}

	/**
	 * Updates the current order by doing a refresh with the database. It will
	 * refresh by the contents of the order search box; it will search again
	 * when there is text written in the search box, or refresh all orders if it
	 * is empty
	 */
	...
	/**
	 * Searches the database for orders by the text written in the order search
	 * box, then updates vthe order list
	 */
	private void searchOrdersBySearchBoxQuery() {
		this.dispatchEvent(new Event<String>(EventType.DATABASE_UPDATE_COOK_GUI_SEARCH_ORDERS_BY_KEYWORDS,
				CookView.orderSearchTextPane.getText().trim()));
	}
	...
	/**
	 * Enables/disables the "mark order in progress" and "Mark order completed"
	 * buttons in the cook GUI according to the state of the passed Order
	 * instance
	 * 
	 * @param order
	 *            The Order instance to use
	 */
	...
	/**
	 * Generates a single string from a list of Extra instances that represents
	 * that list of extras
	 * 
	 * @param extrasList
	 *            An ArrayList containing Extra instances
