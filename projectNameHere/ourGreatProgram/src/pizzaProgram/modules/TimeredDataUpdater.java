 ...
 * views, so that they can update their own data
 * 
...
public class TimeredDataUpdater extends Module implements ActionListener {
	/**
	 * A timer used to determine the time that has passed since the previous
	 * data refresh
	 */
	private Timer updateTimer;
	/**
	 * A list of registered event types that cause a timer reset (the time
	 * should reset each time the GUI is updated)
	 */
	...
	/**
	 * Creates the data refresh timer, and starts it. The start time of the
	 * timer is retrieved as a config value from the database
	 * 
	...
	/**
	 * Adds a specific event type that will represent a data refresh and hence a
	 * timer reset
	 * 
	 * @param eventType
	 *            A String representing the event to be added (MUST appear in
	 *            EventType!)
	 */
	...
	/**
	 * Handles the event of a system data update (either by user interaction or
	 * automatic); resets the timer
	 */
